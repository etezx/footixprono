...
async function loadSchedule(){
  const [scheduleRes, pronosRes, assetsRes] = await Promise.all([
    ...
    fetch(`data/pronos.json?t=${Date.now()}`, {cache:'no-store'}).catch(()=>null),
    fetch(`data/clubs.json?t=${Date.now()}`, {cache:'no-store'}).catch(()=>null)
  ]);
...
async function loadMercato(){
  ...
  try{
    const r=await fetch(`data/mercato.json?t=${Date.now()}`,{cache:'no-store'});
...
async function loadStandings(){
  ...
  if(!body) return;
  try{
    const r=await fetch(`data/standings.json?t=${Date.now()}`,{cache:'no-store'});
