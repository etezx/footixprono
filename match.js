...
let cachedLineups={};
function lineupBlock(home,away,day,meta){const cacheKey=`${day}|||${norm(home)}|||${norm(away)}`;const l=cachedLineups?.matches?.[cacheKey]||realLineups[lineupKey(home,away,day)];if(!l)return `<div class="lineup-empty"><strong>⚽ COMPOSITION V10</strong><p>La composition de cette rencontre n'est pas encore disponible dans le cache Footix. Aucun XI fictif n'est affiché.</p></div>`;if(l.home&&!l.home.name){l.home={...l.home,name:display(home),cls:"home"};l.away={...l.away,name:display(away),cls:"away"}}const official=meta?.completed||meta?.status==="finished";return `<div class="lineup-status"><span class="${official?"official":"probable"}">${official?"✓ XI OFFICIELS":"COMPOSITIONS PROBABLES"}</span><small>${official?"Compositions du match":"En attente des XI officiels"}</small></div><div class="double-pitches">${teamPitchHTML(l.home)}${teamPitchHTML(l.away)}</div>`}
async function hydratePortraits(){await Promise.all([...document.querySelectorAll(".lineup-player")].map(async el=>{let url=el.dataset.portrait||null;if(!url)url=await portrait(el.dataset.player);if(!url)return;const img=new Image();img.alt=el.dataset.player;img.onload=()=>{const w=el.querySelector(".lineup-portrait");w.innerHTML="";w.appendChild(img)};img.src=url}))}
async function init(){try{const [schedule,pronos,standings,clubs,lineups]=await Promise.all([json("schedule.json"),json("pronos.json"),json("standings.json"),json("clubs.json"),json("lineups-v10.json").catch(()=>({matches:{}}))]);cachedLineups=lineups||{matches:{}};const q=new URLSearchParams(location.search);let home=q.get("home")||"STADE RENNAIS FC",away=q.get("away")||"OLYMPIQUE DE MARSEILLE",day=q.get("day")||"4";const fixture=findFixture(schedule,home,away,day);if(fixture){home=fixture.home;away=fixture.away;day=fixture.day}else throw Error("Match introuvable");const meta=fixture.meta||{},cacheKey=`${day}|||${norm(home)}|||${norm(away)}`,cachedMatch=cachedLineups?.matches?.[cacheKey]||{},p=pronoFor(pronos,day,home,away),all=allFixtures(schedule),hf=formFor(home,fixture,all),af=formFor(away,fixture,all),[st,stClass]=status(meta),v=verdict(meta,p);document.title=`${display(home)} – ${display(away)} | Footix Prono`;$("#match-app").innerHTML=`
